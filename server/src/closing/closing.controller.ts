...
export class ClosingController {
  ...
  close(@Body() dto: CreateClosingDto, @GetCurrentUser() currentUser: CurrentUser) {
    return this.closingService.close(dto, currentUser.tenantId!, currentUser.userId)
  }

  @Post(':id/reopen')
  @ApiOperation({ summary: '締め再開' })
  reopen(@Param('id') id: string, @GetCurrentUser() currentUser: CurrentUser) {
    return this.closingService.reopen(id, currentUser.tenantId!, currentUser.userId)
  }
